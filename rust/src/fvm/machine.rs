...
pub unsafe extern "C" fn fil_fvm_machine_execute_message(
    ...
    catch_panic_response(|| {
        ...
        if !apply_ret.exec_trace.is_empty() {
            let mut trace_iter = apply_ret.exec_trace.iter();
...
fn build_lotus_trace(
    ...
    trace_iter: &mut std::slice::Iter<ExecutionEvent>,
...
fn test_lotus_trace() {
    ...
    let return_result = ExecutionEvent::Return(Err(ExecutionError::OutOfGas));
    let trace = vec![
        call_event.clone(),
        call_event.clone(),
        return_result.clone(),
