...
/// * `flattened_public_keys_len` - length of the array
#[no_mangle]
pub unsafe extern "C" fn hash_verify(
    signature_ptr: *const u8,
...
#[no_mangle]
pub unsafe extern "C" fn create_zero_signature() -> *mut types::ZeroSignatureResponse {
...
#[no_mangle]
pub unsafe extern "C" fn drop_signature(sig: *mut u8) {
    if !sig.is_null() {
...
mod tests {
    ...
    fn key_verification() {
        unsafe {
            ...
            let public_key = (*private_key_public_key(&private_key[0])).public_key.inner;
            let message = b"hello world";
            let digest = (*hash(&message[0], message.len())).digest.inner;
            let signature = (*private_key_sign(&private_key[0], &message[0], message.len()))
                .signature
                .inner;
            let verified = verify(
                &signature[0],
            ...
            let different_digest = (*hash(&different_message[0], different_message.len()))
                .digest
                .inner;
