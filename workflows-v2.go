...
func WorkflowProofsLifecycleV2(t TestHelper) {
	...
	sealProofType := abi.RegisteredSealProof_StackedDrg2KiBV2
	winningPostProofType := abi.RegisteredPoStProof_StackedDrgWinning2KiBV2
	...
	t.RequireNoError(filepath.Walk(sealedSectorsDir, visitV2(&sealedSectorPaths)))
	t.AssertEqual(1, len(sealedSectorPaths), sealedSectorPaths)

	// no sector cache dirs, either
	var sectorCacheDirPaths []string
	t.RequireNoError(filepath.Walk(sectorCacheRootDir, visitV2(&sectorCacheDirPaths)))
...
func WorkflowRegisteredSealProofFunctionsV2(t TestHelper) {
	sealTypes := []abi.RegisteredSealProof{
		...
		abi.RegisteredSealProof_StackedDrg8MiBV2,
		abi.RegisteredSealProof_StackedDrg512MiBV2,
		abi.RegisteredSealProof_StackedDrg32GiBV2,
		abi.RegisteredSealProof_StackedDrg64GiBV2,
	}
...
func WorkflowRegisteredPoStProofFunctionsV2(t TestHelper) {
	postTypes := []abi.RegisteredPoStProof{
		...
		abi.RegisteredPoStProof_StackedDrgWinning8MiBV2,
		abi.RegisteredPoStProof_StackedDrgWinning512MiBV2,
		abi.RegisteredPoStProof_StackedDrgWinning32GiBV2,
		abi.RegisteredPoStProof_StackedDrgWinning64GiBV2,
		abi.RegisteredPoStProof_StackedDrgWindow2KiBV2,
		abi.RegisteredPoStProof_StackedDrgWindow8MiBV2,
		abi.RegisteredPoStProof_StackedDrgWindow512MiBV2,
		abi.RegisteredPoStProof_StackedDrgWindow32GiBV2,
		abi.RegisteredPoStProof_StackedDrgWindow64GiBV2,
	}
...
func WorkflowGenerateWinningPoStSectorChallengeEdgeCaseV2(t TestHelper) {
	for i := 0; i < 10000; i++ {
		...
		indices2, err := GenerateWinningPoStSectorChallenge(abi.RegisteredPoStProof_StackedDrgWinning2KiBV2, minerID, randomnessFr32[:], eligibleSectorsLen)
		t.RequireNoError(err)
...
func WorkflowGenerateWinningPoStSectorChallengeV2(t TestHelper) {
	for i := 0; i < 10000; i++ {
		...
		indices, err := GenerateWinningPoStSectorChallenge(abi.RegisteredPoStProof_StackedDrgWinning2KiBV2, minerID, randomnessFr32[:], eligibleSectorsLen)
		t.AssertNoError(err)
...
func randActorIDV2() abi.ActorID {
...
func requireTempFileV2(t TestHelper, fileContentsReader io.Reader, size uint64) *os.File {
...
func requireTempDirPathV2(t TestHelper, prefix string) string {
...
func visitV2(paths *[]string) filepath.WalkFunc {
